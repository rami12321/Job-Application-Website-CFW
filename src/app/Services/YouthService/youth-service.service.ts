...
export class YouthServiceService {
  ...
  getYouthByIsEdited(isEdited: boolean): Observable<any[]> {
    const url = `${this.apiUrl}?isEdited=${isEdited}`;
    return this.http.get<any[]>(url); // Fetch youth data filtered by isEdited status
  }
}
